...
export const CloudinaryUploader: React.FC<CloudinaryUploaderProps> = ({ label, currentUrl, onUrlChange, cloudName, uploadPreset, accept = "image/*,.svg" }) => {
      ...
      <div className="card p-4 bg-base-200">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            >
              <UploadCloud size={16}/>
              <span>{isUploading ? 'Uploading...' : `Upload ${isPdf ? 'PDF' : 'Image'}`}</span>
            </label>
            <input
              id={inputId}
              type="file"
            ...
            {!isUploading && !error && <p className="text-xs text-base-content/50 mt-2">Select a file to upload.</p>}
          </div>
        </div>
      </div>
    </div>
